...
const int numeratorTable[20] = {7, 4, 9, 1, 6, 7, 8, 2, 1, 3, 7, 4, 1, 6, 7, 8, 9, 2, 11, 12}; // metric threading ratio tables

const int denominatorTable[20] = {50, 25, 50, 5, 25, 25, 25, 5, 2, 5, 10, 5, 1, 5, 5, 5, 5, 1, 5, 5};

const int numeratorTable2[20] = {1, 3, 1, 3, 3, 7, 1, 9, 1, 11, 3, 7, 1, 9, 1, 11, 3, 13, 7, 3}; // feed ratio tables

const int denominatorTable2[20] = {40, 80, 20, 50, 40, 80, 10, 80, 8, 80, 20, 40, 5, 40, 4, 40, 10, 40, 20, 8};
...
bool checkSynchronization(int64_t desiredSpindleAngle, volatile long long currentSpindlePosition, volatile long long currentLeadscrewPosition, int feedSelect, const int *numeratorTable, const int *denominatorTable);
long long calculateDesiredSpindleAngle(int feedSelect, double leadscrewPitch, int desiredThreadRevolutions);
...
void loop() {
  ...
  if (driveMode) // thread mode
  {
    ...
    if (!enabled && pulseCount > 0) // if thread mode, disabled, count spindle encoder pulses
    {
      ...
      volatile int64_t currentSpindlePosition = spindlePulsePosition;
...
bool checkSynchronization(int64_t desiredSpindleAngle, volatile long long currentSpindlePosition, volatile long long currentLeadscrewPosition, int feedSelect, const int *numeratorTable, const int *denominatorTable)
{
  ...
  int64_t spindleAngle = (currentSpindlePosition - 1) % spindlePulsesPerRev + 1; // Ensure within 1 to spindlePulsesPerRev range
  ...
  double ratio = (double)numeratorTable[feedSelect] / (double)denominatorTable[feedSelect];
  int64_t spindleMovement = static_cast<int>((spindleAngle - 1) * ratio) + 1; // Adjust for zero-based indexing

  // Serial.println(spindleMovement);

  // Check if spindle movement matches the desired spindle angle within tolerance
  if (abs(spindleMovement == desiredSpindleAngle) || abs((spindleMovement * -1) == desiredSpindleAngle))
  {
...
void bresenham()
{ // sends pulses to motor based on pitch ratio, carries remainders
  ...
  while (accumulator >=
         denominator)
  { // sends required motor steps to motor
...
void spindleCount()
{ // keeps track of spindle angle when disabled in thread mode ("thread dial indicator")
...
void modeHandle()
{ // sets pitch ratio based on feedSelect variable
...
void Achange()
{ // validates encoder pulses, adds to pulse variable
...
void rateIncCall(Button::CALLBACK_EVENT event, uint8_t)
{ // increases feedSelect variable on button press
...
void rateDecCall(Button::CALLBACK_EVENT event, uint8_t)
{ // decreases feedSelect variable on button press
...
void halfNutCall(Button::CALLBACK_EVENT event, uint8_t)
{ // sets readyToThread to true on button hold, starts threading

  if (event == Button::PRESSED_EVENT && driveMode && synced && leadscrewAbsolutePosition == threadStart)
  {
    readyToThread = true;
  }
}

void enaCall(Button::CALLBACK_EVENT event, uint8_t)
{ // toggles enabled variable on button press
...
void lockCall(Button::CALLBACK_EVENT event, uint8_t)
{ // toggles lockState variable on button press
...
void threadSyncCall(Button::CALLBACK_EVENT event, uint8_t)
{ // toggles sync status on button hold, when synced, single press first sets start point, when start point is set, single press jogs to start point
  ...
  if (event == Button::HELD_EVENT)
  {
    ...
    {
      lockState = true;
      leadscrewPulsePosition = 0;
      leadscrewAbsolutePosition = 0;
      spindlePulsePosition = 0;
      synced = true;
    }
...
void modeCycleCall(Button::CALLBACK_EVENT event, uint8_t)
{ // toggles between thread / feed modes on button press
...
void jogLeftCall(Button::CALLBACK_EVENT event, uint8_t)
{ // when disabled in thread mode, a single press jogs one "thread" in the specified direction
...
void jogRightCall(Button::CALLBACK_EVENT event, uint8_t)
{ // when disabled in thread mode, a single press jogs one "thread" in the specified direction
